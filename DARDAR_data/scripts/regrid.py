...
def DARDAR_L2toL3_regrid(year,day):
    ...
    iwcdar_ct = np.zeros([nh,nlat,nlon])
    pciwcdar_ct = np.zeros([nh,nlat,nlon])
    ...
    iwpdar_mean = np.zeros([nlat,nlon])
    pciwpdar_mean = np.zeros([nlat,nlon])
    ...
    for ifile in files:
        ...
        iwcdar = nc.variables['iwc_dar'][:,:]
        pciwcdar = nc.variables['pciwc_dar'][:,:]       
