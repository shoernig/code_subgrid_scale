...
def DARDAR_cldclass(year,day):
    ...
    for ifile,ifileDAR in zip(filesCC,filesDAR):
        ...
        #print(precip[166,:],precip_trans[166,:],precipDAR[165],precipDAR_trans[165,:])
        #exit()
        ...
        prec_mask_DARDAR = np.where(precipDAR_trans==0,1,0)
        #prec_mask = np.where((precip_trans!=1) | (precip_trans!=2),1,0)

        mask_total = conv_mask * prec_mask
        mask_total_DAR = conv_mask * prec_mask_DARDAR
        mask_pc_total = np.where(mask_total==1,0,1)
        mask_pc_total_DAR = np.where(mask_total_DAR==1,0,1)
        ...
        iwc_DAR    = iwc_int * mask_total_DAR
        pciwc_DAR  = iwc_int * mask_pc_total_DAR
        ...
        iwc_DAR    = np.ma.masked_less_equal(iwc_DAR,0)
        pciwc_DAR    = np.ma.masked_less_equal(pciwc_DAR,0)
        ...
        opath = '/work/bb1093/b380620/DATA/Data/DARDAR_L2_cldclass_flag/'
        ...
        IWCDAR  =  ncout.createVariable('iwc_dar',np.float32,('time','nbin'))
        PCIWCDAR=  ncout.createVariable('pciwc_dar',np.float32,('time','nbin'))
        ...
        IWCDAR   = metadata(IWCDAR,'kg m-3',-999.99,'mass concentration of frozen water in air - DARDAR precip flag','')
        PCIWCDAR  = metadata(PCIWCDAR,'kg m-3',-999.99,'mass concentration of frozen water in air (precipitating & convective) - DARDAR precip flag','')
